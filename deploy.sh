...
kubectl apply -f adabas-db-deployment.yaml \
  -f adabas-db-service.yaml \
  -f employees-service-deployment.yaml \
  -f employees-service.yaml

# https://github.com/kubernetes/kubernetes/issues/27081#issuecomment-238078103
kubectl patch deployment adabas-db-deployment \
  -p "{\"spec\":{\"template\":{\"metadata\":{\"labels\":{\"date\":\"`date +'%s'`\"}}}}}"
kubectl patch deployment employees-service-deployment \
  -p "{\"spec\":{\"template\":{\"metadata\":{\"labels\":{\"date\":\"`date +'%s'`\"}}}}}"
